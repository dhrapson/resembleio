		...
		Describe("reading a valid config yaml", func() {
			...
			Context("when it contains a full HTTP type", func() {
				...
				It("should return a configuration", func() {
					...
					Expect(err).NotTo(HaveOccurred())
					Expect(config.TypeName).To(Equal("HTTP"))
					httpRequestMatcher := config.MatcherConfigs[0]
					Expect(httpRequestMatcher.PathRegexString).To(Equal("/test"))
					Expect(httpRequestMatcher.VerbRegexString).To(Equal("GET|POST"))
					Expect(httpRequestMatcher.HostRegexString).To(Equal("localhost"))
					Expect(httpRequestMatcher.QueryParams[0].KeyRegexString).To(Equal("guid"))
					Expect(httpRequestMatcher.QueryParams[0].ValueRegexString).To(Equal("[a-zA-Z-0-9-]*"))
					Expect(httpRequestMatcher.QueryParams[1].KeyRegexString).To(Equal("abc"))
					Expect(httpRequestMatcher.QueryParams[1].ValueRegexString).To(Equal("123"))
				})
			})
		})
	})
