...
var _ = Describe("HostHttpMatcher", func() {

	var (
		regex   string
		matcher HostHttpMatcher
		err     error
	)
	...
	Describe("when using a valid regex", func() {
		...
		JustBeforeEach(func() {
			matcher, err = NewHostHttpMatcher(regex)
		})

		Context("when given an exactly matching regexp", func() {

			BeforeEach(func() {
				regex = `^myhost.exact.com$`
			})

			It("should return true", func() {
				Expect(err).NotTo(HaveOccurred())
				result := matcher.MatchHost("myhost.exact.com")
		...
		Context("when given a loosely matching regexp", func() {
			...
			It("should return true", func() {
				Expect(err).NotTo(HaveOccurred())
				result := matcher.MatchHost("myhost.example.com")
				Expect(result).To(BeTrue())
			})
		})

		Context("when given a non-matching regexp", func() {
			...
			It("should return false", func() {
				Expect(err).NotTo(HaveOccurred())
				result := matcher.MatchHost("thathost.com")
